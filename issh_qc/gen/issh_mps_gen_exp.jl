...
function expect_err_den(psi)
    ...
    for (bs_ind, j) in enumerate(ind_arr)
        ...
        proj_zz = proj_op_zz(psi, j, j+1)
        proj_oo = proj_op_oo(psi, j, j+1)

        push!(expect_m, expectation_proj_new(psi, j, j+1, proj_zz) + expectation_proj_new(psi, j, j+1, proj_oo))
    end

    push!(expect_m, expectation_proj_new(psi, 1, N, proj_op_zz(psi, 1, N)) + expectation_proj_new(psi, 1, N, proj_op_oo(psi, 1, N)))
